...
arbi_T		Arbi_FromInt(l_int i);
l_int	*Arbi_ToInt(arbi_T x);
...
#define WordSize 32
#define HalfSize 16
#define Hmask (~(~((unsigned) 0) << (unsigned) HalfSize))
#define s_Word int
#define s_HalfWord short int
#define MinWord (((int) 1) << (WordSize-1))
#define MaxWord (~MinWord)
#define MinHalf (((short) 1) << HalfSize)
#define MaxHalf (~MinHalf)
#define Abs(x) ((x>=0) ? (x) : -(x))
#endif
...
About conversion routines:
    ...
    arbi_FromInt(l_int i)
	Convert the l_int i to an arbi_T.
	No failures.
    arbi_ToInt(arbi_T n)
	Returns a pointer to a l_int with the same value as the arbi_T n.
	arbi_ToInt fails if n doesn't fit in a l_int.
